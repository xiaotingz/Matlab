% function [face_dist_avgs, face_dist_stds] = calcDistFromTargetNormals(file, faces, target_normals, eps_curv, eps_area, eps_min_ang)
% ###########################################################################
% * Input
...
% tri_dists = calcTriDistFromTargetNormals(tri_normals, tri_fls, ea, target_normals);
...
faces = sort(faces, 2);
face_dist_avgs = ones(size(faces, 1), size(target_normals, 1)) * NaN;
face_dist_stds = ones(size(faces, 1), size(target_normals, 1)) * NaN;
