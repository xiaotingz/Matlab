% load data
clear
file = ('/Users/xiaotingzhong/Desktop/091616_STO_1470/101116_V4_misA3/sub2_misA3_recons1Again2_curv.dream3d');
...
boolean1 = data_raw(1,:) > 0 & data_raw(2,:) > 0 & ~isnan(data_raw(3,:)) & data_raw(3,:) < 10 & data_raw(3,:) > -10;
...
holder = zeros(length(labelList_use),3);
j = 1;  % j -- jth faces
...
for i = 1:length(data_sorted)-1
    ...
    if data_sorted(i,1) == data_sorted(i+1,1) && data_sorted(i,2) == data_sorted(i+1,2)
        ...
        holder(j,1) = data_sorted(i,1);
        holder(j,2) = data_sorted(i,2);
        holder(j,3) = totalA;
        holder(j,4) = totalCA;
        holder(j,5) = disA_use(j);
...
holder(j,1) = data_sorted(i+1,1);
holder(j,2) = data_sorted(i+1,2);
holder(j,3) = totalA + data_sorted(i+1,4);
holder(j,4) = totalCA + data_sorted(i+1,3)*data_sorted(i+1,4);
holder(j,5) = disA_use(j);

%% get rid of grain faces between surface grains
bool = ones(length(holder),1);
for i = 1:length(holder)
%     the index of surf_grain = grainID + 1
    gA = holder(i,1) + 1;
    gB = holder(i,2) + 1;
...
tmp2 = holder(:,4);
tmp3 = holder(:,5);
data_final(:,1) = tmp1(bool);
data_final(:,2) = tmp2(bool);
data_final(:,3) = tmp3(bool);
